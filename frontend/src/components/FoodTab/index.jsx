...
const MobileTab = () => {
  ...
  return (
    <>
      <div className="lg:hidden bg-white shadow-lg flex fixed bottom-0 z-10 w-full items-center md:justify-evenly text-gray-500 border">
        {allTypes.map((item) => {
          return (
            <Link key={item.id} to={`/${item.id}`} className="w-1/4">
              ...
              >
                <div
                  className={
                    type === item.id
                      ? "w-full h-full border-t-2 border-zomato-400 pt-2 flex flex-col items-center"
